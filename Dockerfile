...
# Fetch NS-3 source
RUN wget http://www.nsnam.org/release/ns-allinone-3.40.tar.bz2
RUN tar -xf ns-allinone-3.40.tar.bz2

# Configure and compile NS-3
RUN cd ns-allinone-3.40 && ./build.py --enable-examples --enable-tests
...
RUN apt-get clean && \
  rm -rf /var/lib/apt && \
  rm /usr/ns-allinone-3.40.tar.bz2